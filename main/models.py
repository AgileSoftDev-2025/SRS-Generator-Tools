...
class GUI(models.Model):
    id_gui = models.CharField(max_length=20, primary_key=True)
    project = models.ForeignKey(Project, on_delete=models.CASCADE, related_name='guis')
    nama_atribut = models.CharField(max_length=50)

    class Meta:
        # Satu project hanya punya satu GUI default
        constraints = [
            models.UniqueConstraint(fields=['project'], name='unique_gui_per_project')
        ]
...
class UserStory(models.Model):
    ...
    # RC-03 fix: tambah FK langsung ke Project untuk isolasi data yang lebih kuat
    project = models.ForeignKey('Project', on_delete=models.CASCADE, null=True, blank=True, related_name='user_stories')
    ...
    class Meta:
        # Satu project tidak boleh punya kombinasi actor+fitur yang duplikat
        constraints = [
            models.UniqueConstraint(
                fields=['project', 'input_sebagai', 'input_fitur'],
                name='unique_userstory_per_project'
            )
        ]
...
class UseCaseSpecification(models.Model):
    ...
    # Isolasi per project — field ini wajib agar data tidak bocor antar project
    project = models.ForeignKey(Project, on_delete=models.CASCADE, null=True, blank=True, related_name='use_case_specs')
    gui = models.ForeignKey(GUI, on_delete=models.CASCADE, null=True, blank=True)
    ...
    class Meta:
        # Satu project tidak boleh punya feature_name yang duplikat
        constraints = [
            models.UniqueConstraint(
                fields=['project', 'feature_name'],
                name='unique_spec_per_project'
            )
        ]
...
class ImportedTable(models.Model):
    # Isolasi per project — hapus unique=True agar nama tabel bisa sama di project berbeda
    project = models.ForeignKey(Project, on_delete=models.CASCADE, null=True, blank=True, related_name='imported_tables')
    name = models.CharField(max_length=255)

    class Meta:
        # Satu project tidak boleh punya tabel dengan nama sama
...
class Feature(models.Model):
    # RC-03 fix: tambah FK ke Project agar Feature tidak menjadi data orphan global
    project = models.ForeignKey('Project', on_delete=models.CASCADE, null=True, blank=True, related_name='features')
    nama = models.CharField(max_length=200)

    class Meta:
        constraints = [
            models.UniqueConstraint(fields=['project', 'nama'], name='unique_feature_per_project')
        ]
...
class SqlTable(models.Model):
    id = models.AutoField(primary_key=True)
    # RC-03 fix: tambah FK ke Project agar data SQL tidak bocor antar project
    project = models.ForeignKey('Project', on_delete=models.CASCADE, null=True, blank=True, related_name='sql_tables')
