...
def parse_sql_file(sql_content):
    ...
    for stmt in statements:
        ...
        # --- FIX 1: Regex CREATE TABLE yang sangat fleksibel (mendukung IF NOT EXISTS dan schema.table) ---
        match = re.search(
            r'CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?[`"]?(?:[a-zA-Z0-9_]+[`"]?\.[`"]?)?([a-zA-Z0-9_]+)[`"]?\s*\((.*?)\)\s*;?',
            stmt_clean,
            re.S | re.I
        )
        if not match:
